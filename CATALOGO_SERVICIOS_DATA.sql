...
FROM (
    SELECT
        ...
        CASE
            ...
            WHEN IFNULL(p.EffectiveFilter2, 1) = 2 THEN CASE
            ...
            END
            WHEN IFNULL(p.EffectiveFilter2, 1) = 3 THEN CONCAT(FORMAT(IFNULL(srv.ProductComission, 0),0), '%')
            WHEN IFNULL(p.EffectiveFilter2, 1) = 4 THEN CONCAT(CAST(ROUND(IFNULL(srv.ProductSlots, 0) * 5, 0) AS SIGNED), 'min')
            WHEN IFNULL(p.EffectiveFilter2, 1) = 5 THEN CASE
                WHEN IFNULL(srv.ProductRequiresDoctor, 0) IN (-1, 1) THEN 'Si'
                ELSE 'No'
            END
            WHEN IFNULL(p.EffectiveFilter2, 1) = 6 THEN IFNULL(NULLIF(TRIM(srv.ProductSketches), ''), '-')
