...
FROM (
    SELECT
        ...
        CASE
            WHEN IFNULL(cfg.HasTariff, 0) = 0 THEN CONVERT('-' USING utf8mb4) COLLATE utf8mb4_unicode_ci
            WHEN IFNULL(srv.HasActiveService, 0) = 0 THEN CONVERT('-' USING utf8mb4) COLLATE utf8mb4_unicode_ci
            WHEN IFNULL(p.EffectiveFilter2, 1) = 2 THEN CONVERT(
                CASE
                    WHEN srv.ProductVAT IS NULL THEN '-'
                    ELSE CONCAT(
                        FORMAT(
                            CASE
                                WHEN ABS(IFNULL(srv.ProductVAT, 0)) <= 1 THEN IFNULL(srv.ProductVAT, 0) * 100
                                ELSE IFNULL(srv.ProductVAT, 0)
                            END,
                            0
                        ),
                        '%'
            ...
            WHEN IFNULL(p.EffectiveFilter2, 1) = 3 THEN CONVERT(CONCAT(FORMAT(IFNULL(srv.ProductComission, 0),0), '%') USING utf8mb4) COLLATE utf8mb4_unicode_ci
            WHEN IFNULL(p.EffectiveFilter2, 1) = 4 THEN CONVERT(CONCAT(CAST(ROUND(IFNULL(srv.ProductSlots, 0) * 5, 0) AS SIGNED), 'min') USING utf8mb4) COLLATE utf8mb4_unicode_ci
            WHEN IFNULL(p.EffectiveFilter2, 1) = 5 THEN CONVERT(
                CASE
            ...
            WHEN IFNULL(p.EffectiveFilter2, 1) = 6 THEN CONVERT(IFNULL(NULLIF(TRIM(srv.ProductSketches), ''), '-') USING utf8mb4) COLLATE utf8mb4_unicode_ci
            ELSE CONVERT(CONCAT('$', FORMAT(IFNULL(srv.ProductPrice, 0), 0)) USING utf8mb4) COLLATE utf8mb4_unicode_ci
...
LEFT JOIN (
    SELECT
        ...
        MAX(CASE WHEN IFNULL(d.ProductDisabled, 0) = 0 THEN d.ProductVAT END) AS ProductVAT,
