...
FROM (
    SELECT
        ...
        CASE
            WHEN IFNULL(bon.HasTariff, 0) = 0 THEN CONVERT('-' USING utf8mb4) COLLATE utf8mb4_unicode_ci
            WHEN IFNULL(bon.HasActiveBonus, 0) = 0 THEN CONVERT('-' USING utf8mb4) COLLATE utf8mb4_unicode_ci

            WHEN IFNULL(p.EffectiveFilter2, 1) = 2 THEN
                CONVERT(
                    CONCAT(
                        FORMAT(
                            CASE
                                WHEN ABS(IFNULL(bon.ProductCostPriceVAT, 0)) <= 1 THEN IFNULL(bon.ProductCostPriceVAT, 0) * 100
                                ELSE IFNULL(bon.ProductCostPriceVAT, 0)
                            END,
                            0
                        ),
                        '%'
            ...
            ELSE
                CONVERT(CONCAT('$', FORMAT(IFNULL(bon.ProductPrice, 0), 0)) USING utf8mb4) COLLATE utf8mb4_unicode_ci
...
INNER JOIN (
    ...
    WHERE (
            d.ProductType IN (2, 5, 9)
            OR IFNULL(d.ProductVoucherUnits, 0) > 0
            OR IFNULL(d.ProductVoucherNum, 0) > 0
            OR IFNULL(d.ProductVoucherPID, 0) > 0
            OR IFNULL(d.ProductVoucherZCID, 0) > 0
    )
) bonus_parents
    ON bonus_parents.ParentProductID = parent.ProductID
...
LEFT JOIN (
    SELECT
        ...
        1 AS HasTariff,
        MAX(CASE WHEN IFNULL(d.ProductDisabled, 0) = 0 THEN 1 ELSE 0 END) AS HasActiveBonus,
    ...
    ON bon.ParentProductID = parent.ProductID
   AND bon.ClinicID = corp.ClinicID
