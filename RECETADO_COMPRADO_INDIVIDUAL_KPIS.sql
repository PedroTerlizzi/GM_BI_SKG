...
SELECT
    k.KPIID AS `KPIID`,
    CASE k.KPIID
        WHEN 1 THEN 'Consultas en Flowww'
        WHEN 2 THEN 'Recetas Digitales'
        WHEN 3 THEN 'Productos Recetados'
        WHEN 4 THEN 'Disponibles'
        WHEN 5 THEN 'Comprados'
    END AS `KPIName`,
    CASE k.KPIID
        WHEN 1 THEN CAST(a.ConsultasFlowww AS CHAR)
        WHEN 2 THEN CAST(a.Recetas AS CHAR)
        WHEN 3 THEN CAST(a.ProductosRecetados AS CHAR)
        WHEN 4 THEN CONCAT(ROUND(CASE
            WHEN a.ProductosRecetados = 0 THEN 0
            ELSE (a.ProductosDisponibles * 100.0) / a.ProductosRecetados
            END,1),'%')
        WHEN 5 THEN CONCAT(ROUND(CASE
            WHEN a.ProductosRecetados = 0 THEN 0
...
FROM (
    SELECT
        x.Recetas,
        x.ProductosRecetados,
        x.ProductosDisponibles,
        x.ProductosComprados,
        (
            SELECT COUNT(*)
            FROM laser_gen lg
            WHERE lg.LaserGUserID = x.EffectiveUserID
              AND FIND_IN_SET(lg.LaserGClinicID, x.EffectiveClinicIDs)
              AND lg.LaserGDate >= x.EffectiveStartDate
              AND lg.LaserGDate < DATE_ADD(x.EffectiveEndDate, INTERVAL 1 DAY)
        ) AS ConsultasFlowww
    FROM (
        SELECT
            ...
            MAX(r.EffectiveUserID) AS EffectiveUserID,
            MAX(r.EffectiveClinicIDs) AS EffectiveClinicIDs,
            MAX(r.EffectiveStartDate) AS EffectiveStartDate,
            MAX(r.EffectiveEndDate) AS EffectiveEndDate
        FROM (
            SELECT
                bg.BudgetGID,
                p.EffectiveUserID,
