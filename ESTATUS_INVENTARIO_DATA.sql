...
SELECT
    ...
    d.ProductDesc AS `Descripción Producto`,
    ...
    END AS `Costo Un`,
    d.FamilyParentName AS `Família`,
    d.ProviderName AS `Proveedor`,
    CAST(ROUND(d.CantidadInicial, 0) AS SIGNED) AS `Cantidad Inicial`,
    CAST(ROUND(d.CantidadEntradasManuales, 0) AS SIGNED) AS `Entradas Manuales`,
    CAST(ROUND(d.CantidadOtrasEntradas, 0) AS SIGNED) AS `Otras Entradas`,
    CAST(ROUND(d.CantidadSalidasVentas, 0) AS SIGNED) AS `Salidas por Ventas`,
    CAST(ROUND(d.CantidadOtrasSalidas, 0) AS SIGNED) AS `Otras Salidas`,
