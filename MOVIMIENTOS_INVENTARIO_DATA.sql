...
SELECT
    r.StockRID AS `ID Registro`,
    r.StockRDate AS `Fecha`,
    r.StockRTime AS `Hora`,
    r.TipoRegistroID AS `Tipo de Registro (ID)`,
    r.TipoRegistroDesc AS `Tipo de Registro (Desc)`,
    r.Sucursal AS `Sucursal`,
    r.ProductDesc AS `Descripcion Producto`,
    r.StockRProductLot AS `Lote`,
    r.StockRExpiryDate AS `Caducidad`,
    r.FamilyParentID AS `FamilyParentID`,
    r.FamilyParentName AS `Familia de Productos`,
    ...
    CAST(r.IVA AS DECIMAL(18,2)) AS `IVA`,
    CAST(r.CostoUnitario AS DECIMAL(18,4)) AS `Costo Unitario (c/ IVA)`,
    CAST((r.CantidadInicial * r.CostoUnitario) AS DECIMAL(18,2)) AS `Valor Inicial (a costo c/ IVA)`,
    CAST((r.CantidadMovimiento * r.CostoUnitario) AS DECIMAL(18,2)) AS `Valor Movimiento (a costo c/ IVA)`,
    CAST((r.CantidadFinal * r.CostoUnitario) AS DECIMAL(18,2)) AS `Valor Final (a costo c/ IVA)`
FROM (
    SELECT
        sr.StockRID,
