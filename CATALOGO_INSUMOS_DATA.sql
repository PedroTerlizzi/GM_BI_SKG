...
FROM (
    SELECT
        ...
        CASE
            WHEN IFNULL(sup.HasActiveSupply, 0) = 0 THEN CONVERT('-' USING utf8mb4) COLLATE utf8mb4_unicode_ci

            WHEN IFNULL(p.EffectiveFilter2, 2) = 1 THEN
                CONVERT(CAST(CAST(ROUND(IFNULL(sup.StockUnits, 0), 0) AS SIGNED) AS CHAR) USING utf8mb4) COLLATE utf8mb4_unicode_ci

            WHEN IFNULL(p.EffectiveFilter2, 2) = 2 THEN
                CONVERT(CONCAT('$', FORMAT(IFNULL(sup.ProductCostPrice, 0), 2)) USING utf8mb4) COLLATE utf8mb4_unicode_ci
            ...
            WHEN IFNULL(p.EffectiveFilter2, 2) = 3 THEN
                CONVERT(
                    CONCAT(
                        FORMAT(
                            CASE
                                WHEN ABS(IFNULL(sup.ProductCostPriceVAT, 0)) <= 1 THEN IFNULL(sup.ProductCostPriceVAT, 0) * 100
                                ELSE IFNULL(sup.ProductCostPriceVAT, 0)
                            END,
                            0
                        ),
                        '%'
            ...
            WHEN IFNULL(p.EffectiveFilter2, 2) = 4 THEN
                CONVERT(
                    CASE
                        WHEN IFNULL(sup.ProductRequiresTraceability, 0) IN (-1, 1) THEN 'Si'
                        ELSE 'No'
                    END
                    USING utf8mb4
                ) COLLATE utf8mb4_unicode_ci

            WHEN IFNULL(p.EffectiveFilter2, 2) = 5 THEN
                CONVERT(
                    CASE
                        WHEN IFNULL(sup.ProductType, 0) = 3 THEN 'Si'
                        ELSE 'No'
                    END
                    USING utf8mb4
                ) COLLATE utf8mb4_unicode_ci
...
CROSS JOIN (
    ...
    FROM (
        ...
        /* Debug defaults (used automatically in local execution) */
            ...
            '124,125,126,127,128,129,130,131,132,133,134,135,136,137,138,139' AS DebugFilter1,
            2                                        AS DebugFilter2,
...
LEFT JOIN (
    SELECT
        ...
        MAX(CASE WHEN IFNULL(d.ProductDisabled, 0) = 0 THEN 1 ELSE 0 END) AS HasActiveSupply,
        MAX(CASE WHEN IFNULL(d.ProductDisabled, 0) = 0 THEN d.ProductCostPrice END) AS ProductCostPrice,
        MAX(CASE WHEN IFNULL(d.ProductDisabled, 0) = 0 THEN d.ProductCostPriceVAT END) AS ProductCostPriceVAT,
        MAX(CASE WHEN IFNULL(d.ProductDisabled, 0) = 0 THEN d.ProductRequiresTraceability END) AS ProductRequiresTraceability,
        MAX(CASE WHEN IFNULL(d.ProductDisabled, 0) = 0 THEN d.ProductType END) AS ProductType,
        SUM(CASE WHEN IFNULL(d.ProductDisabled, 0) = 0 THEN IFNULL(st.StockUnits, 0) ELSE 0 END) AS StockUnits
    ...
    LEFT JOIN stock st
        ON st.StockProductID = d.ProductID
       AND st.StockClinicID = pg.ProductGClinicID
    WHERE d.ProductType IN (3, 6)
    GROUP BY
        d.ProductParentID,
        pg.ProductGClinicID
) sup
    ON sup.ParentProductID = parent.ProductID
   AND sup.ClinicID = corp.ClinicID
