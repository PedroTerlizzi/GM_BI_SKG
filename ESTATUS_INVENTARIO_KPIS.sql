...
SELECT
    ...
    CASE k.KPIID
        ...
        WHEN 2 THEN 'Entradas'
        WHEN 3 THEN 'Salidas'
        WHEN 4 THEN 'Inventario Final'
    END AS `KPIName`,
    CONCAT(
        CASE
            WHEN (
                CASE k.KPIID
                    WHEN 1 THEN IFNULL(a.InventarioInicial, 0)
                    WHEN 2 THEN IFNULL(a.Entradas, 0)
                    WHEN 3 THEN IFNULL(a.Salidas, 0)
                    WHEN 4 THEN IFNULL(a.InventarioFinal, 0)
                    ELSE 0
                END
            ) < 0
                THEN '-$'
